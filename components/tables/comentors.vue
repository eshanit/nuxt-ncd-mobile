...
const rows = computed(() => {
    return props.comentors.filter((comentor: any) => {
        return Object.values(comentor).some((value) => {
...
const onSubmit = (a: Array<String>) => {
    useComentors.saveGoingCoMentors(a);
};


/**endform */

const countSelected = (dataArray: Array<boolean>) => {
    return dataArray.filter(Boolean).length
}


const deleteCoMentor = async (comentorId: string, comentorRev: string, firstname: string) => {
     const e = await useComentors.deleteCoMentor(comentorId, comentorRev)
...
defineEmits(['showCreateUserForm', 'comentorData'])

onUpdated(() => {
    useComentors.fetchCoMentors()
})
</script>
        ...
        <template #header>
            <div class="flex justify-between">
                <div class="text-sm text-cyan-700">
                    <strong>Co-Mentors: <span class="text-rose-500">{{ comentors.length }}</span></strong>
                </div>
                <div>
                    <UButton variant="soft" color="purple" size="lg" @click="$emit('showCreateUserForm', true)">
                        <strong>Add Co-Mentor</strong>
                    </UButton>
                </div>
            </div>
        </template>
        <template #body>
            <v-form @submit="onSubmit(values)">
                ...
                <div class="flex justify-between pt-3 px-3 pb-3.5 border-b border-gray-200 dark:border-gray-700">
                    <div>
                        <UInput v-model="q" placeholder="Filter Co-Mentors..." />
                    </div>
                ...
                <UTable :columns="columns" :rows="rows">
                    <template #actions-data="{ row }">
                        <div class="grid grid-cols-3 gap-1">
                            <div>
                                <UCheckbox :id="row._id" :value="row._id" v-model="values" />
                            </div>
                            <div>
                                <UIcon name="i-heroicons-pencil-square" class=" text-green-500 text-xl "  @click="$emit('comentorData', {
                                    id: row._id, 
                                    rev: row._rev, 
                                                    ...
                                                    <div class="flex" v-if="!showSuccessMessage">
                                                        <div class="pr-2">
                                                            <UButton variant="outline" color="sky" label="Yes"
                                                                @click=" deleteCoMentor(row._id, row._rev, row.firstname); close()" />
                                                        </div>
                                                        <div>
                ...
                <div class="flex justify-end px-3 py-3.5 border-t border-gray-200 dark:border-gray-700">
                    <UPagination v-model="page" :page-count="pageCount" :total="comentors.length" />
                </div>

                <div class="border-t pt-5" v-show="countSelected(Object.values(values)) > 0">
